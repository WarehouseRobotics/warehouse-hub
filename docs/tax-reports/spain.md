...
Agents need a stable Spanish tax-position view derived from ingested tax reports. This can be exposed as a future summary endpoint or as the expected output of `tax-reports get/list` aggregation, but the semantics should be fixed now.
...
* VAT position comes from the latest filed `303` for the fiscal year, optionally checked against `390`.
* Autónomo YTD profit/loss comes from `130` `casilla 03`, not from invoices or expenses.
* Corporate profit/loss position comes from `200` facts. `currentYearProfitOrLoss` should use taxable-base facts when available, with accounting result kept separately.
* Remaining compensable negative tax base comes from `200` page-15 negative-base detail, not from a recalculation.
